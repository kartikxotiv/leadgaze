...
export default function ContactDetailsPage({
  ...
  const { data: contact, isLoading, isError } = useSalesContact(params.id);
  ...
  return (
    <DashboardLayout>
      <div className="w-full bg-white min-h-screen">
        {/* Refined Header */}
        <div className="w-full bg-gray-100/80 border-b border-gray-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="w-full p-4 space-y-4">
          ...
          <Collapsible open={isAboutOpen} onOpenChange={setIsAboutOpen}>
            <CollapsibleTrigger className="flex items-center gap-2 w-full p-2 bg-gray-100 rounded-md hover:bg-gray-200 transition-colors group border border-gray-200">
              <ChevronDown
                className={cn(
            ...
            <CollapsibleContent className="space-y-0 px-1 pt-1">
              ...
              />
              <DetailRow label="Title" value={contact.title || "Owner"} />
              <DetailRow label="Owner" value={contact.owner || "-"} />
              <DetailRow label="Reports To" value={contact.reports_to || "-"} />
              <DetailRow
                label="Description"
                value={contact.comment || "Test"}
              />
              <DetailRow
                label="Contact Owner"
                value={
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      href="#"
                      className="text-blue-600 hover:underline decoration-dashed decoration-blue-400 underline-offset-2"
                    >
                      Mayank Ghildiyal
                    </Link>
                  </div>
                ...
                icon={
                  <div className="flex items-center gap-2 text-blue-600">
                    <User className="h-4 w-4 fill-current opacity-30" />
                ...
                value={
                  <div className="space-y-3">
                    <div className="flex flex-col text-blue-600">
                      <span>{contact.location == "Andorra" ? "Test" : ""}</span>
...
function DetailRow({
  ...
  return (
    <div className="group flex items-start justify-between py-3 border-b border-gray-200 last:border-0 px-2">
      <div className="flex-1 space-y-1">
        <h3 className="text-[13px] font-semibold text-gray-800">{label}</h3>
        <div className="text-[14px] font-normal text-gray-900 min-h-[22px]">
          {value}
        </div>
      </div>
