-- ============================================================
-- 1. Drop existing overloaded functions to avoid "is not unique"
-- ============================================================
...
DROP FUNCTION IF EXISTS public.get_core_meetings(UUID, TEXT, UUID, TEXT[], TEXT, TEXT, UUID[], TIMESTAMPTZ, TIMESTAMPTZ, TIMESTAMPTZ, TIMESTAMPTZ, BOOLEAN, UUID, INT, INT, TEXT, TEXT, UUID, TEXT, BOOLEAN, BOOLEAN, UUID, TEXT);
DROP FUNCTION IF EXISTS public.get_core_email_activity(UUID, TEXT, UUID, INT, INT, UUID, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.get_core_tasks(UUID, TEXT, UUID, TEXT);
...
BEGIN
  ...
  WITH matching_tasks AS (
    SELECT DISTINCT t.id, t.workspace_id, t.title, t.description, t.due_date as due_at, t.completed_at,
           (CASE WHEN t.is_completed THEN 'completed' ELSE 'open' END) as status, t.priority, t.created_by, t.assigned_to, t.created_at, t.updated_at,
           rel.entity_type as rel_entity_type, rel.entity_id as rel_entity_id
    FROM core.tasks t
