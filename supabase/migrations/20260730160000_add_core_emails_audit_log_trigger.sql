-- Add audit log trigger for core.emails table

-- 1. Update fn_audit_log_trigger to handle 'core_emails' and 'emails' entity name
...
-- 2. Add Trigger on core.emails
DROP TRIGGER IF EXISTS tr_audit_log_core_emails ON core.emails;
CREATE TRIGGER tr_audit_log_core_emails
  AFTER INSERT OR UPDATE OR DELETE ON core.emails
  FOR EACH ROW EXECUTE FUNCTION public.fn_audit_log_trigger('core_emails', 'common');
