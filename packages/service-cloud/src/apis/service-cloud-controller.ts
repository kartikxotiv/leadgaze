...
export const getServiceCloudResourceController = catchAsync(async ({ request, params }) => {
  const resource = params?.resource ?? '';
  const config = getResourceConfig(resource);
  if (!config) return NextResponse.json({ success: false, message: 'Unknown resource' }, { status: 404 });
  ...
  if (!workspaceId) return NextResponse.json({ success: false, message: 'workspaceId is required' }, { status: 400 });

  const { supabase, user, error } = await assertServiceCloudWorkspaceAccess(workspaceId);
  ...
  if (id) {
    const { data, error: fetchError } = await query.eq('id', id).maybeSingle();
...
export const createServiceCloudResourceController = catchAsync(async ({ request, params }) => {
  const resource = params?.resource ?? '';
  const config = getResourceConfig(resource);
  if (!config) return NextResponse.json({ success: false, message: 'Unknown resource' }, { status: 404 });
  ...
  return NextResponse.json({ success: true, message: `${resource} created successfully`, data }, { status: 201 });
});

export const updateServiceCloudResourceController = catchAsync(async ({ request, params }) => {
  const resource = params?.resource ?? '';
  const config = getResourceConfig(resource);
  if (!config) return NextResponse.json({ success: false, message: 'Unknown resource' }, { status: 404 });
...
export const deleteServiceCloudResourceController = catchAsync(async ({ request, params }) => {
  const resource = params?.resource ?? '';
  const config = getResourceConfig(resource);
  if (!config) return NextResponse.json({ success: false, message: 'Unknown resource' }, { status: 404 });
...
export const getServiceCloudDashboardController = catchAsync(async ({ request }) => {
  const url = new URL(request.url);
  const workspaceId = url.searchParams.get('workspaceId') ?? url.searchParams.get('workspace_id');
  if (!workspaceId) return NextResponse.json({ success: false, message: 'workspaceId is required' }, { status: 400 });
  ...
  const [
    ...
    client.from('tickets').select('id', { count: 'exact', head: true }).eq('workspace_id', workspaceId).eq('is_deleted', false),
    client.from('tickets').select('id', { count: 'exact', head: true }).eq('workspace_id', workspaceId).eq('is_deleted', false).is('closed_at', null),
    client.from('customers').select('id', { count: 'exact', head: true }).eq('workspace_id', workspaceId).eq('is_deleted', false),
    client.from('organizations').select('id', { count: 'exact', head: true }).eq('workspace_id', workspaceId).eq('is_deleted', false),
    client.from('time_entries').select('duration_seconds').eq('workspace_id', workspaceId),
    client.from('tickets').select('*').eq('workspace_id', workspaceId).eq('is_deleted', false).order('created_at', { ascending: false }).limit(8),
  ]);

  const totalLoggedSeconds = (timeEntries.data ?? []).reduce(
    (sum: number, entry: { duration_seconds?: number }) => sum + Number(entry.duration_seconds ?? 0),
    0,
  );

  return successDataResponse('Service Cloud dashboard retrieved successfully', {
