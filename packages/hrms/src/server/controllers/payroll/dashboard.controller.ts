...
export const listPayrollDashboardController = catchAsync(async ({ user }) => {
    ...
    payrollRuns:
      payrollRuns?.map((item: any) => {
        ...
        const grossEarnings = runEntries.reduce(
          (sum: number, entry: any) => sum + Number(entry.gross_earnings ?? 0),
          0,
        );
        ...
        return {
          ...
          breakdown: runEntries.map((entry: any) => ({
            ...
            status: entry.status,
            items: (entry.payroll_entry_items ?? []).map((entryItem: any) => ({
              id: entryItem.id,
