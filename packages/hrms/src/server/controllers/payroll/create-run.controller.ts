...
export const createPayrollRunController = catchAsync(
  async ({ body, request, user }) => {
    ...
    const { data: existingRun } = await hrms
      .from('payroll_runs')
      .select('*')
      .eq('workspace_id', workspaceId)
    ...
    const { data: assignments, error: assignmentsError } = await hrms
    ...
    if (existingRun) {
      ...
      await hrms.from('payroll_entries').delete().eq('payroll_run_id', runId);
    } else {
      const { data: createdRun, error: runError } = await hrms
        .from('payroll_runs')
        .insert({
    ...
    for (const assignment of assignments) {
      const { data: assignmentComponents, error: componentsError } = await hrms
      ...
      const actualEnd =
        empEffectiveTo && empEffectiveTo < periodEnd
          ? empEffectiveTo
          : periodEnd;
      const actualDays =
      ...
      const { data: payItems } = await hrms
      ...
      const { data: entry, error: entryError } = await hrms
      ...
      for (const itemPayload of entryItemPayloads) {
        const { error: itemInsertError } = await hrms
