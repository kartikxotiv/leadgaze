...
export const getRecruitmentDashboardController = catchAsync(
  async ({ request, user }) => {
      ...
      candidates: candidates.map((candidate: any) => {
        ...
        return {
          ...
          latest_feedback_summary:
            candidateLatestFeedback.get(candidate.id) ?? null,
    ...
    return successDataResponse(
      'Recruitment dashboard fetched successfully',
      response,
    );
  },
);

export const getRecruitmentOptionsController = catchAsync(
  async ({ request, user }) => {
    const supabaseAdmin = getSupabaseServerAdminClient<any>();
    const hrms = getRecruitmentHrmsClient(supabaseAdmin);
    const userId = getRecruitmentUserId(user);
