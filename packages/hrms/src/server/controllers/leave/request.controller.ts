...
const createLeaveRequestController = catchAsync(
  async ({ body, request, user }) => {
    ...
    const { data: overlappingRequests, error: overlappingError } = await hrms
      .from('leave_requests')
      .select('id')
    ...
    const { data: yearRequests, error: yearRequestsError } = await hrms
      .from('leave_requests')
...
const updateLeaveRequestController = catchAsync(
  async ({ body, params, request, user }) => {
    ...
    if (actionBody.action === 'cancel') {
      if (
        !context.employee ||
        leaveRequest.employee_id !== context.employee.id
      ) {
        throw new ApiError('Forbidden', 403);
      }
      ...
      payload.status =
        actionBody.action === 'approve' ? 'approved' : 'rejected';
      payload.decision_at = new Date().toISOString();
