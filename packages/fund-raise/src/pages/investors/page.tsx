    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-4xl">
      ...
      <div className="grid gap-6">
        ...
        <section className="grid gap-3">
          <h3 className="font-semibold custom-sub-heading-dialog-form">Contacts</h3>
        ...
        <section className="grid gap-3"><h3 className="font-semibold custom-sub-heading-dialog-form">Related Deals</h3><div className="rounded-md border">{relatedDeals.length === 0 ? <div className="p-4 text-sm text-muted-foreground">No related deals.</div> : relatedDeals.map((deal) => <div key={deal.id} className="flex justify-between border-b p-3 text-sm last:border-b-0"><span>{dealDisplay(deal)}</span><span>{deal.status}</span></div>)}</div></section>
      </div>
    </DialogContent>
  );
}
