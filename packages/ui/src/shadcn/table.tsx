...
const TableHead: React.FC<React.ThHTMLAttributes<HTMLTableCellElement>> = ({
  ...
  <th
    className={cn(
      'table-header-border table-row-border primary-text-medium text-leadgaze-dark first:border-l-0 last:border-r-0 h-[42px] px-4 text-left align-middle [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]',
      className,
    )}
    {...props}
  />
...
const TableCell: React.FC<React.TdHTMLAttributes<HTMLTableCellElement>> = ({
  ...
  <td
    className={cn(
      'table-row-border h-[42px] px-4 py-0 align-middle [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]',
      className,
    )}
    {...props}
  />
