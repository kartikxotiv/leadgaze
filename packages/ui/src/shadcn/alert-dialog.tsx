...
> = ({ className, ...props }) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      className={cn(
        'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-2 border-0 shadow-lg duration-200 dark:border',
        className,
      )}
      {...props}
    />
...
const AlertDialogFooter = ({
  ...
  <div
    className={cn(
      'flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 dialog-footer-border-top custom-spacing-x-y dialog-footer',
      className,
    )}
    {...props}
  />
...
> = ({ className, ...props }) => (
  <AlertDialogPrimitive.Description
    className={cn('text-leadgaze-dark primary-text-medium custom-spacing-x-y py-2 dark:text-white', className)}
    {...props}
  />
);
