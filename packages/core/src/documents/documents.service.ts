...
import { GetDocumentsParams, DocumentItem } from './documents.types';
import { CorePaginatedResponse } from '../reminders/reminders.types';

export class DocumentsService {
  constructor(private readonly supabase: SupabaseClient) {}

  async getDocuments(params: GetDocumentsParams): Promise<CorePaginatedResponse<DocumentItem>> {
    const {
      workspaceId,
      entityType,
      entityId,
    ...
    const { data, error } = await (this.supabase as any).rpc('get_core_documents', {
    ...
    return (data as CorePaginatedResponse<DocumentItem>) || { data: [], total: 0, page: 1, limit: 0, has_more: false };
  }
}
