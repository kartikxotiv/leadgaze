...
export const coreGoogleCallbackController = catchAsync(async ({ request }) => {
  ...
  const fallbackUrl = new URL(
    '/home/core/email-settings?error=oauth_error',
    request.url,
  );
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL('/home/core/email-settings?error=missing_params', request.url),
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL('/home/core/email-settings?error=invalid_state', request.url),
    );
  }
  ...
  if (!userInfo.email) {
    return NextResponse.redirect(
      new URL('/home/core/email-settings?error=auth_failed', request.url),
    );
  }
    ...
    .upsert(
      {
        ...
        // Reconnect fix: clear soft-delete markers, errors, and stale sync
        // state so the account is fully reactivated and visible again.
        last_error: null,
        settings: {},
        last_synced_at: null,
        history_id: null,
  ...
  if (dbError) {
    return NextResponse.redirect(
      new URL('/home/core/email-settings?error=db_error', request.url),
    );
  }

  return NextResponse.redirect(
    new URL(decodedState.returnUrl || '/home/core/email-settings', request.url),
  );
});
