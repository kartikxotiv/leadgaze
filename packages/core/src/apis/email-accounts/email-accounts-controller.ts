...
export const getCoreEmailAccountsController = catchAsync(async ({ request }) => {
...
export const createCoreSmtpAccountController = catchAsync(async ({ request }) => {
  const url = new URL(request.url);
  const workspaceId = url.searchParams.get('workspace_id') ?? url.searchParams.get('workspaceId');
  ...
  if (!body?.email || !body?.host || !body?.port || !body?.username || !body?.password) {
    return NextResponse.json({ success: false, message: 'SMTP email, host, port, username, and password are required' }, { status: 400 });
  }
  ...
  const accessScope = memberContext.isAdmin && body.access_scope === 'workspace' ? 'workspace' : 'private';
    ...
    .upsert(
      {
        ...
        imap_password: body.imap_password ? encrypt(body.imap_password) : encrypt(body.password),
...
export const updateCoreEmailAccountController = catchAsync(async ({ request }) => {
  const body = await request.json().catch(() => null);
  const { id, workspace_id: workspaceId, workspaceId: camelWorkspaceId, is_active, access_scope } = body ?? {};
  ...
  const supabase = getSupabaseServerClient();
  const memberContext = await getWorkspaceMemberContext(supabase, resolvedWorkspaceId);
  ...
  if (!existingAccount) {
    return NextResponse.json({ success: false, message: 'Email account not found' }, { status: 404 });
  }
  ...
  if (typeof access_scope !== 'undefined' && !memberContext.isAdmin) {
    return NextResponse.json({ success: false, message: 'Only admins can change account access' }, { status: 403 });
  }

  const updatePayload: Record<string, unknown> = { updated_by: memberContext.userId };
...
export const deleteCoreEmailAccountController = catchAsync(async ({ request }) => {
  const url = new URL(request.url);
