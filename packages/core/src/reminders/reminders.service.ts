...
import { GetRemindersParams, ReminderItem, CorePaginatedResponse } from './reminders.types';

export class RemindersService {
  constructor(private readonly supabase: SupabaseClient) {}

  async getReminders(params: GetRemindersParams): Promise<CorePaginatedResponse<ReminderItem>> {
    const {
      workspaceId,
      entityType,
      entityId,
    ...
    const { data, error } = await (this.supabase as any).rpc('get_core_reminders', {
    ...
    return (data as CorePaginatedResponse<ReminderItem>) || { data: [], total: 0, page: 1, limit: 0, has_more: false };
  }
}
