...
export const getCoreWorkspaceEmailActivityService = asyncHandlerClient(
  async (
    workspaceId: string,
    limit = 20,
    offset = 0,
...
export const getCoreEntityEmailActivityService = asyncHandlerClient(
  async (
    workspaceId: string,
    entityType: string,
    entityId: string,
...
export const saveCoreEmailActivityService = asyncHandlerClient(
...
export const deleteCoreEmailActivityService = asyncHandlerClient(
  async (id: string, workspaceId: string) => {
    const response = await CoreApiClient.delete(
      `/email-activity?id=${id}&workspaceId=${workspaceId}`,
    );
    return response.data?.data ?? null;
  },
);

export const sendCoreEmailService = asyncHandlerClient(
  async (payload: Record<string, unknown>) => {
...
export const syncCoreEmailAccountsService = asyncHandlerClient(
  async (payload: {
    workspaceId: string;
