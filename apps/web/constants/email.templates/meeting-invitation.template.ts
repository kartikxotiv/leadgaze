const MEETING_INVITATION_TEMPLATE = ({
  meetingTitle,
  meetingDescription,
  startTime,
  endTime,
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  <div style="max-width: 600px; margin: 40px auto; background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    ...
    <div style="background-color: #3953E7; background: linear-gradient(135deg, #3953E7 0%, #283BA4 100%); padding: 32px 24px; text-align: center;">
      <div style="background-color: rgba(255,255,255,0.2); display: inline-block; padding: 6px 14px; border-radius: 20px; margin-bottom: 12px;">
        <span style="color: #ffffff; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px;">You're Invited</span>
      </div>
      <p style="margin: 0; color: #ffffff; font-size: 24px; font-weight: bold; line-height: 1.2;">📅 Meeting Invitation</p>
    </div>
    ...
    <div style="padding: 32px 24px;">
      ...
      ${
        hostName || hostEmail
          ? `<p style="margin: 0 0 20px 0; color: #4a5568; font-size: 14px;">
              Hosted by <strong style="color: #1a202c;">${hostName || hostEmail}</strong>${hostName && hostEmail ? ` (${hostEmail})` : ''}
            </p>`
          : ''
      }
        ...
        <table style="width: 100%; border-collapse: collapse;">
          <tr>
            <td style="padding: 8px 0;">
              <p style="margin: 0; color: #283BA4; font-size: 13px; font-weight: 600; text-transform: uppercase;">Start Time</p>
              <p style="margin: 4px 0 0 0; color: #1a202c; font-size: 16px; font-weight: 500;">${formatDate(startTime)}</p>
            </td>
          </tr>
          <tr>
            <td style="padding: 8px 0;">
              <p style="margin: 0; color: #283BA4; font-size: 13px; font-weight: 600; text-transform: uppercase;">End Time</p>
      ...
      <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #e2e8f0;">
        <p style="margin: 0; color: #718096; font-size: 12px; line-height: 1.5;">
          You have been invited to this meeting via ${productName}.
        </p>
      </div>
    </div>
...
export default MEETING_INVITATION_TEMPLATE;
