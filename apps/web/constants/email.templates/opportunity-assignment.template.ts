const OPPORTUNITY_ASSIGNMENT_EMAIL_TEMPLATE = ({
  opportunityName,
  accountName,
  amount,
  currency = 'USD',
...
<head>
  ...
  <title>You have been assigned an Opportunity - ${productName}</title>
</head>
    ...
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 12px rgba(0, 0, 0, 0.08);">
      ...
      <div style="background-color: #ffffff; padding: 32px 24px; text-align: center; border-bottom: 4px solid #3953E7;">
        ...
        <h1 style="margin: 0 0 8px 0; font-size: 24px; font-weight: 700; color: #1a1a1a; line-height: 1.3;">
          You have been assigned an Opportunity
        </h1>
        <p style="margin: 0; font-size: 16px; font-weight: 600; color: #3953E7;">${opportunityName}</p>
      </div>
      ...
      <div style="padding: 32px 24px;">
        ...
        <p style="margin: 0 0 24px 0; font-size: 15px; line-height: 1.6; color: #424242;">
          ${
            assignerName
              ? `<strong>${assignerName}</strong> has assigned you to manage the opportunity <strong>${opportunityName}</strong> on <strong>${productName}</strong>.`
              : `You have been assigned to manage the opportunity <strong>${opportunityName}</strong> on <strong>${productName}</strong>.`
          }
        </p>

        <!-- Opportunity Details Card -->
        <div style="background-color: #f8f9fc; border: 1px solid #e0e7ff; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
          <p style="margin: 0 0 12px 0; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #3953E7;">Opportunity Details</p>
          <p style="margin: 0 0 8px 0; font-size: 15px; color: #1a1a1a;"><strong>Opportunity Name:</strong> ${opportunityName}</p>
          ${
            accountName
              ? `<p style="margin: 0 0 8px 0; font-size: 15px; color: #1a1a1a;"><strong>Account:</strong> ${accountName}</p>`
              : ''
          }
          ${
            formattedAmount
              ? `<p style="margin: 0; font-size: 15px; color: #1a1a1a;"><strong>Value:</strong> ${formattedAmount}</p>`
              : ''
          }
        </div>
        ...
        <div style="text-align: center; margin: 0 0 24px 0;">
          <a href="${opportunityUrl}" style="display: inline-block; background-color: #3953E7; color: #ffffff; text-decoration: none; padding: 12px 32px; border-radius: 6px; font-size: 15px; font-weight: 700; border: none; cursor: pointer; box-shadow: 0 4px 12px rgba(57, 83, 231, 0.3);">
            View Opportunity
          </a>
        </div>
        ...
        <div style="background-color: #f0f4ff; border-left: 4px solid #3953E7; padding: 14px; border-radius: 4px;">
          <p style="margin: 0; font-size: 12px; line-height: 1.6; color: #424242;">
            Please log in to your ${productName} workspace to view full stage progress, activity logs, or update opportunity details.
          </p>
        </div>
      </div>
...
export default OPPORTUNITY_ASSIGNMENT_EMAIL_TEMPLATE;
