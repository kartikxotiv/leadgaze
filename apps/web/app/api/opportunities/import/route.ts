...
const importOpportunities = catchAsync(
    ...
    const insertPayloads = data.map((row: any) => {
      ...
      if (cleanedRow.stage_id) {
        const match = stages.find(
          (s) =>
            s.status_name.toLowerCase() === cleanedRow.stage_id.toLowerCase(),
        );
      ...
      // 7. Sanitize Numbers
      if (cleanedRow.amount !== undefined)
        cleanedRow.amount = sanitizeNumber(cleanedRow.amount);
      if (cleanedRow.probability !== undefined)
        cleanedRow.probability = sanitizeNumber(cleanedRow.probability);
