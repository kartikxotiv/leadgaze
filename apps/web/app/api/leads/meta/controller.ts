...
export const getLeadsMeta = catchAsync(async ({ request }: { request: NextRequest }) => {
  ...
  try {
    ...
    // 3. Fetch CRM Module ID from public schema
    ...
    if (moduleError || !module) throw new Error('moduleError: ' + JSON.stringify(moduleError));

    // 4. Fetch Lead Statuses from public schema
    const { data: statuses, error: statusesError } = await adminClient.schema('public')
    ...
    if (statusesError) throw new Error('statusesError: ' + JSON.stringify(statusesError));

    // Transform fields to match the frontend EntityField shape
    ...
    console.error('getLeadsMeta Error:', err);
    throw err;
  }
});
