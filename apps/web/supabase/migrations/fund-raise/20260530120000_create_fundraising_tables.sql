...
-- =====================================================
-- 7. Register Fundraising Module in Shared RBAC
-- =====================================================

INSERT INTO public.crm_modules (module_key, module_name, description, display_order, is_system)
VALUES
  ('fundraising', 'Fundraising', 'Manage funding rounds, investor pipeline, and deal tracking', 20, TRUE)
ON CONFLICT (module_key) DO NOTHING;

-- Sub-modules (parent = fundraising)
INSERT INTO public.crm_modules (module_key, module_name, description, display_order, is_system, parent_module_id)
SELECT
  sub.module_key,
  sub.module_name,
  sub.description,
...
CROSS JOIN (
  SELECT 'fundraising_investors' AS module_key, 'Investors'      AS module_name, 'Manage investor organisations and contacts' AS description, 1 AS display_order
  UNION ALL
  SELECT 'fundraising_rounds',                  'Funding Rounds', 'Create and manage funding rounds',                                        2
  UNION ALL
  SELECT 'fundraising_pipeline',                'Pipeline',       'Track deals across pipeline stages',                                      3
) sub
WHERE m.module_key = 'fundraising'
ON CONFLICT (module_key) DO NOTHING;
